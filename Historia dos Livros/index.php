    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-dark" onclick="continuarCompra()">Continuar Compra</button>
...
function adicionarAoCarrinho(nomeLivro, precoLivro) {
  ...
  if (livroExistente) {
    // Se o livro já estiver no carrinho, incrementa a quantidade
    livroExistente.quantidade++;
  } else {
    // Se o livro não estiver no carrinho, adiciona um novo item
    carrinho.push({ nome: nomeLivro, preco: precoLivro, quantidade: 1 });
  }

  // Atualizando o conteúdo do carrinho no modal
  atualizarConteudoCarrinho();

  // Exibindo o alerta (opcional)
  const mensagem = `${nomeLivro} adicionado ao carrinho!`;
  alert(mensagem);

  // Exibindo o carrinho no console para fins de depuração
...
function atualizarConteudoCarrinho() {
  ...
  if (carrinho.length === 0) {
    ...
    // Adicionando linhas com itens do carrinho
...
function finalizarCompra() {
  // Lógica para finalizar a compra (pode ser adicionada conforme necessário)
  alert('Compra Continuada! Redirecionando para a página de Checkout.');

  // Exibindo o carrinho no console para fins de depuração
  console.log("Carrinho antes de enviar para o PHP:", carrinho);

  // Atualize o campo oculto antes do redirecionamento
  var carrinhoInput = document.getElementById('carrinhoInput');
  carrinhoInput.value = JSON.stringify(carrinho);

  // Abra o modal de carrinho
...
function continuarCompra() {
  ...
  window.location.href = 'http://localhost/Historia%20dos%20Livros/Checkout.php';
}
